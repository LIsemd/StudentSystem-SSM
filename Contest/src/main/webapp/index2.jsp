...
<!-- 搭建教师端显示页面 -->
	...
	<font color=black>
		...
		<div class="col-md-offset-10">
			...
			<button type="button" class="btn btn-info btn-sm" id="change_modal_btn">
				<span class="glyphicon glyphicon-hand-right" aria-hidden="true">
					修改已审核对象
				</span>
			</button>
		...
		<div class="row">
			<div class=" col-md-offset-2 col-md-8 ">
				<table class="table table-hover" id="ches_table" >
					<thead>
						<tr>
		...
		<div class="row">
			<!--分页文字信息  -->
			<div class="col-md-offset-2 col-md-6" id="page_info_area"></div>
			<!-- 分页条信息 -->
			<div class="col-md-offset-2" id="page_nav_area">
	...
	<script type="text/javascript">
		...
		function build_ches_table(result){
			//清空table表格
			$("#ches_table tbody").empty();
			var ches = result.extend.pageInfo.list;
			$.each(ches,function(index,item){
				...
				.append($("<span></span>").addClass("glyphicon glyphicon-pencil")).append("批准");
				checkBtn.attr("check-id",item.stuIdTd);
				var delBtn = $("<button></button>").addClass("btn btn-danger btn-sm delete_btn")
				.append($("<span></span>").addClass("glyphicon glyphicon-trash")).append("否决");
				delBtn.attr("del-id",item.stuIdTd);
				var btnTd = $("<td></td>").append(checkBtn).append(" ").append(delBtn);
		...
		$(document).on("click",".check_btn",function(){
			...
			if(confirm("你确认【"+name+"】嘛ヾ(´∀`o)+")){
				$.ajax({
					url:"${APP_PATH}/confirm/"+IdTd,
					type:"POST",
					data:IdTd,
					success:function(result){
						alert(result.msg);
		...
		$("#change_modal_btn").click(function(){
